...
async function loadHistoricalData(chartId, sensorName, hours = 1) {
  ...
  async function fetchMPULatest() {
    ...
    try {
      ...
      const stateIndicator = document.getElementById('userStateIndicator');
      const stateTextEl = document.getElementById('userStateText');
      if (stateIndicator && stateTextEl) {
        const stateIndex = parseInt(data.state, 10);
        let currentState = { text: 'Indefinido', class: 'state-unknown' };

        // ALTERADO: Lógica para agrupar em 3 níveis de atividade
        if (stateIndex === 0) {
          currentState = { text: 'Parado', class: 'state-idle' };
        } else if (stateIndex >= 1 && stateIndex <= 3) {
          currentState = { text: 'Em Movimento', class: 'state-active' };
        } else if (stateIndex === 4) {
          currentState = { text: 'Queda Detectada', class: 'state-alert' };
        }
        
        stateTextEl.textContent = `Estado: ${currentState.text}`;
        // Remove classes antigas e adiciona a nova para estilização
        stateIndicator.className = 'state-indicator ' + currentState.class;
      }
