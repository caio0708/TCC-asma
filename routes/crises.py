...
def treinar_e_avaliar_modelos():
    ...
    e salva o pipeline completo (pré-processamento + modelo).
    """
    try:
    ...
    # Define os modelos a serem treinados usando Pipelines para incluir a normalização
    pipelines = {
    ...
    # Salva o melhor modelo (pipeline completo)
    if best_model:
        print(f"\nMelhor modelo: {best_model_name} com acurácia de {best_accuracy:.4f}")
        os.makedirs(MODELO_CRISE_DIR, exist_ok=True)
        joblib.dump(best_model, MODELO_CRISE_PATH)
        print(f"Melhor modelo salvo em: {MODELO_CRISE_PATH}")
...
try:
    ...
    best_model = joblib.load(MODELO_CRISE_PATH)
    # Para obter as colunas, precisamos carregar os dados de qualquer maneira
    df_train = pd.read_csv(file_path)
    X_train_cols = pd.get_dummies(df_train.drop(["Patient_ID", "Has_Asthma", "Asthma_Control_Level"], axis=1), drop_first=True).columns
    # A acurácia não é salva, então definimos como N/A ou recalculamos se necessário
    best_accuracy = "N/A (modelo carregado)"
    ...
    if best_model is None:
        ...
        # Define para evitar erros de referência
...
def predict_crisis(new_patient: dict, model_to_use, train_columns) -> int:
    ...
    """
    if not model_to_use or train_columns.empty:
