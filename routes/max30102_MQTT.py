...
SERIAL_PORT = 'COM7' # Verifique se a porta está correta
BAUD_RATE   = 115200
TIMEOUT_S   = 1
SAMPLE_RATE = 100.0
...
def compute_spo2(comp, cal):
    ...
    R, PI = comp["R"], comp["PI"]
    print(f"DEBUG: Valor R puro = {R:.4f}, PI = {PI:.2f}%") 
...
# =============================================================================
def iniciar_monitor_sensores(on_data_callback):
    """
    Inicia o monitoramento com lógica de calibração avançada e chama o callback com novos dados.
    """
    ...
    if CAL.load():
        print("[Monitor Calibrado] ✅ Calibração carregada de", CALIB_FILE)
    else:
        print("[Monitor Calibrado] ℹ️  Sem arquivo de calibração. Usando coeficientes padrão.")

    ser = None
    print("[Monitor Calibrado] Iniciando...")
        ...
        while True:
            ...
            if not is_connected:
                try:
                    ...
                    print(f"[Monitor Calibrado] ✅ Conectado a {SERIAL_PORT}")
                    is_connected = True
                ...
                try:
                    if ser.in_waiting > 0:
                        ...
                        try:
                            ...
                            # Adicionado para ler temperatura e umidade também
                            elif ':' in line:
                    ...
                    print(f"[Monitor Calibrado] ❌ Dispositivo em {SERIAL_PORT} desconectado.")
                    if ser: ser.close()
                    ser, is_connected = None, False
                    ir_buffer.clear(); red_buffer.clear(); bpm_history.clear(); spo2_history.clear()

            # --- LÓGICA DE PROCESSAMENTO SUBSTITUÍDA ---
            ...
            # Prepara os dados e chama o callback
            dados_para_enviar = {
            ...
            sleep_time = 0.05 if is_connected else 3.0
            time.sleep(sleep_time)

    except KeyboardInterrupt:
        print("\n[Monitor Calibrado] Encerrado.")
    finally:
