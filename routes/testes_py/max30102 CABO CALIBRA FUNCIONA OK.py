# -*- coding: utf-8 -*-
"""
MAX30102 Oximetria de Pulso - Algoritmo para Pulseira (v7.3 R-scale + anchor)
------------------------------------------------------------------------------
- SpO2: adiciona spo2_R_scale (escala do R) e âncora exata com 1 ponto.
- ≥2 pontos: reestima A/B sobre R escalado (limites fisiológicos).
- Anti-latch: perda de sinal -> "Sem sinal" + reset dos filtros.
...
class CalibrationStore:
    def __init__(self):
        ...
        self.spo2_R_scale = 1.0                 # << NOVO
        # BPM
        self.hr_scale = 1.0; self.hr_bias = 0.0
        # Pontos
        self.spo2_points = []                   # {"R_mean":..., "SpO2_ref":...}
        self.hr_points   = []                   # {"bpm_meas":..., "bpm_ref":...}
    ...
    def fit_spo2(self):
        """Ajusta parâmetros de SpO2 usando pontos coletados.
           1 ponto -> ajusta R_scale + offset de ancoragem;
           ≥2 pontos -> ajusta A/B sobre R escalado.
        """
        if len(self.spo2_points) >= 2:
            ...
            # S ≈ A − B·R_eff  => S ≈ (-R_eff)*B + A
            m, c = self._linear_fit(-R_eff, S)  # m=B, c=A
...
def compute_spo2(comp):
    """Retorna: spo2_clamped, R (bruto), conf, R_eff, spo2_unclamped."""
...
def keyboard_watcher():
    ...
    while True:
        if msvcrt.kbhit():
            ...
            elif ch in ('r','R'): KEYS["reset"] = True   # reset calibração
            elif ch in ('q','Q'): KEYS["quit"] = True; break
        time.sleep(0.03)

# ==================== CALIBRAÇÃO ====================
def perform_calibration(ser, duration_s=15):
    t_end = time.time() + duration_s
    R_list = []; bpm_meas_list = []
    print("\n🔧 Calibração: fique em repouso por ~{} s e leia os valores no OXÍMETRO DE DEDO.".format(duration_s))
    while time.time() < t_end:
        if ser.in_waiting > 0:
            line = ser.readline().decode('utf-8', errors='ignore').strip()
            process_serial_line(line)
        if len(ir_buffer) >= SHORT_SIZE:
            ir_raw = list(ir_buffer)[-BUFFER_SIZE:]; red_raw = list(red_buffer)[-BUFFER_SIZE:]
...
def display_panel(bpm_disp, spo2_disp, r_val, r_eff, spo2_unc, pi_val, conf_bpm, conf_spo2, signal_ok):
    ...
    print("═"*84)
    print("   MONITOR CARDÍACO & OXIMETRIA (MAX30102) - v7.3 R-scale + anchor")
    print("═"*84)
...
# ==================== MAIN ====================
def main():
    print("Iniciando... Conectando à porta serial...")
    try:
        ser = serial.Serial(SERIAL_PORT, BAUD_RATE, timeout=TIMEOUT_S)
        print(f"✅ Conectado a {SERIAL_PORT}")
    except serial.SerialException as e:
        print(f"❌ ERRO: Não foi possível conectar. Verifique porta/conexões.\n   {e}")
        return
    ...
    try:
        while True:
            ...
            if KEYS["reset"]:
                # Reset "limpo" da calibração (sem recriar objeto para não perder referências)
            ...
            if KEYS["calibrate"]:
                perform_calibration(ser); KEYS["calibrate"] = False

            bpm_disp = None; spo2_disp = None
            r_val = None; r_eff = None; spo2_unc = None
            pi_val = None
            conf_bpm = 0.0; conf_spo2 = 0.0
            signal_ok = False
            now = time.time()
            ...
            if len(ir_buffer) >= BUFFER_SIZE:
                ...
                # Sinal "ok" quando PI e amplitude satisfazem limites
                if comp is not None:
                    pi_val = comp["PI"]
                ...
                if spo2_meas is not None and signal_ok:
                    ...
                    # sem medida válida → após hold, zera
