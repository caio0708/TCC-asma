...
CLASS_MAP_PATH = "/Users/mac/Downloads/TCC-asma-main-2/dados/yamnet_class_map.csv"
...
print("[Detector de Tosse] A carregar o mapa de classes local...")
try:
    with open(CLASS_MAP_PATH, 'r') as f:
        class_map_csv_text = f.read()
    print("[Detector de Tosse] Mapa de classes carregado com sucesso.")
except FileNotFoundError:
    print(f"[Detector de Tosse] Erro: O arquivo '{CLASS_MAP_PATH}' não foi encontrado.")
    print("[Detector de Tosse] Por favor, certifique-se de que o arquivo existe no caminho especificado.")
    exit()
except Exception as e:
    print(f"[Detector de Tosse] Erro ao ler o mapa de classes: {e}")
    exit()
...
def iniciar_detector_tosse(on_cough_detected_callback):
    ...
    try:
        ...
        with sd.InputStream(
            # Garante que o microfone externo seja usado
            # device=2, # ID 0 para External Microphone e 2 pra mic padrao
            channels=1,
