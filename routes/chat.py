...
# Importações atualizadas e corretas do LangChain
...
cache = TTLCache(maxsize=100, ttl=600) # Cache de 10 minutos para dados ambientais

def analisar_qualidade_ar(aqi):
    """Analisa o Índice de Qualidade do Ar (AQI) e retorna uma estrutura de dados."""
    recomendacao = ""
    nivel = ""
    emoji = ""
    # Mapeamento do AQI (1-5) para níveis e recomendações
    if aqi == 1:
        nivel = "Bom"
        emoji = "🟢"
        recomendacao = "Qualidade do ar ideal. Aproveite para atividades ao ar livre."
    elif aqi == 2:
        nivel = "Razoável"
        emoji = "🟡"
        recomendacao = "Qualidade do ar aceitável. Pessoas sensíveis devem limitar esforços prolongados."
    elif aqi == 3:
        nivel = "Moderado"
        emoji = "🟠"
        recomendacao = "Pessoas com asma podem sentir desconforto. Evite atividades intensas ao ar livre."
    elif aqi == 4:
        nivel = "Ruim"
        emoji = "🔴"
        recomendacao = "Alerta de saúde. Evite exposição ao ar livre. Mantenha as janelas fechadas."
    else: # aqi_value == 5
        nivel = "Muito Ruim"
        emoji = "🟣"
        recomendacao = "Risco elevado à saúde. Permaneça em ambientes internos com purificadores de ar, se possível."
...
def analisar_umidade(humidity):
    """Analisa a umidade e retorna uma estrutura de dados."""
    recomendacao = ""
    nivel = ""
    emoji = ""
    if 30 <= humidity <= 50:
        nivel = "Ideal"
        emoji = "🟢"
        recomendacao = "Níveis de umidade ideais para a respiração."
    elif humidity > 50:
        nivel = "Alta"
...
def analisar_temperatura(temp):
    """Analisa a temperatura e retorna uma estrutura de dados."""
    recomendacao = ""
    nivel = ""
    emoji = ""
    if 18 <= temp <= 22:
        nivel = "Agradável"
        emoji = "🟢"
        recomendacao = "Temperatura confortável, sem grandes riscos para a asma."
    elif temp < 18:
        nivel = "Fria"
...
logging.info("Iniciando carregamento do chatbot especialista...")
if not os.path.exists(FAISS_INDEX_PATH):
    raise FileNotFoundError(f"Índice '{FAISS_INDEX_PATH}' não encontrado. Execute 'criar_base_de_conhecimento.py'.")

# Use um modelo de embedding leve e eficiente, como o nomic-embed-text
...
def dados_ambientais():
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            temperatura_amb  = next(s["valor"] for s in lista_sensores if s["id"] == "temperatura-ambiente")
            t, humidity = future_weather.result() 

        # Gera as análises estruturadas
        sugestoes_list = [
            analisar_qualidade_ar(aqi),
            analisar_temperatura(temperatura_amb),
...
def sugestoes_iniciais():
    try:
        prompt = (
            ...
            "Sempre nas perguntas deixe explicito que se trata sobre um paciente asmatico ou uma duvida com relação a asma" #melhorar aq dps
        )
